...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $multaCliente = $_POST['multa'];
    $cpfAntigo = "";

    $link = mysqli_connect("localhost", "root", "udesc", "biblioteca");

    $stmt = mysqli_prepare($link, "SELECT cpf FROM clientes WHERE idCliente = ?");
    ...
    $codVerificaCpf = verificaCPF($cpfCliente);

    if($multaCliente < 0){
        $_SESSION['msg'] = "Ulitilize valores maiores que 0 para multa";
        $_SESSION['msgCOD'] = 1;
        $_SESSION['codWhere'] = 3;
    ...
    switch ($codVerificaCpf) {
        case 1:
            ...
            exit;
        case 2:
            if($cpfAntigo != $cpfCliente){
                $_SESSION['msg'] = "Já existe um cliente com seu cpf";
                $_SESSION['msgCOD'] = 1;
                $_SESSION['codWhere'] = 3;
                header('Location: ../administrador/gerenciarUsuarios.php');
                exit;
    ...
    $stmt = mysqli_prepare($link, "UPDATE clientes SET nome = ?, cpf = ?, multa = ? WHERE idCliente = ?");
    
    mysqli_stmt_bind_param($stmt, "ssdi", $nomeCliente, $cpfCliente, $multaCliente, $idCliente);
